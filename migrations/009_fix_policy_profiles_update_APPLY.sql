-- ============================================================================
-- 009_fix_policy_profiles_update_APPLY.sql — APLICACAO REAL (commit)
-- Rodar SOMENTE com ordem explícita do coach e backup do dia verde.
--
...
--    O ator vive só entre SAVEPOINT e ROLLBACK TO — nunca persiste, sem
--    DELETE (o protocolo do apply-migration.mjs proíbe DELETE, com razão).
-- ---------------------------------------------------------------------------
savepoint prova_pre;
...
reset role;
rollback to savepoint prova_pre;   -- descarta o ator; o fix vem a seguir
...
-- ---------------------------------------------------------------------------
-- 3) PROVA PÓS-FIX (ator recriado sob novo savepoint; o fix acima fica)
-- ---------------------------------------------------------------------------
savepoint prova_pos;
...
reset role;
rollback to savepoint prova_pos;   -- descarta o ator; persiste APENAS o fix
